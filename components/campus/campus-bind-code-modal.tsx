...
interface CampusBindCodeModalProps {
  onClose: () => void
  onSuccess: () => void
}

export function CampusBindCodeModal({ onClose, onSuccess }: CampusBindCodeModalProps) {
  ...
  const handleBind = async () => {
    ...
    try {
      const res = await fetch('/api/campus/code/bind', {
        method: 'POST',
      ...
      setAdminName(data.data?.classAdmin?.displayName || data.data?.classAdmin?.username || '班级管理员')
      setStep('success')
          ...
          <div className="px-6 py-6 space-y-5">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-primary/5 border border-primary/10">
              ...
              <p className="text-sm text-muted-foreground">
                请输入班级管理员提供给您的校区邀请码，绑定后将自动获取该管理员名下所有班级的查看权限。
              </p>
            </div>
          ...
          <div className="px-6 py-8 flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                已成功绑定管理员 <span className="font-medium text-foreground">{adminName}</span>
              </p>
