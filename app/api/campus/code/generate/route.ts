...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user || user.role !== 'classadmin') {
      return errorResponse('Only class admins can generate campus bind codes', 403)
    }
    ...
    let existing = await prisma.campusAdminBind.findFirst({ where: { inviteCode } })
    while (existing) {
      inviteCode = generateInviteCode()
      existing = await prisma.campusAdminBind.findFirst({ where: { inviteCode } })
    }

    const existingSource = await prisma.campusAdminBind.findFirst({
      where: { classAdminId: userId, campusAdminId: null, status: 'active' },
    })

    if (existingSource) {
      const updated = await prisma.campusAdminBind.update({
    ...
    const source = await prisma.campusAdminBind.create({
      data: {
        classAdminId: userId,
        inviteCode,
        validPeriod,
    ...
    console.error('Generate campus code error:', error)
    return errorResponse('Failed to generate campus bind code', 500)
  }
}
