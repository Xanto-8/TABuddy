...
export class DeepseekService {
  ...
  async generateText(
    ...
    if (!apiKey) {
      this.logger.warn('DEEPSEEK_API_KEY not configured, returning fallback text')
      return this.getFallbackText(userPrompt)
    }
    ...
    try {
      ...
      this.logger.error(`DeepSeek API call failed: ${(error as Error).message}`)
      return this.getFallbackText(userPrompt)
    }
  }

  async generateNotificationCopy(scenePrompt: ScenePromptRequest): Promise<string> {
...
请生成一句简洁的通知文案。`
    ...
    return this.generateText(systemPrompt, userPrompt, 0.85, 128)
  }

  private getFallbackText(prompt: string): string {
