(function () {
  ...
  var CSS =
    ...
    '#tabuddy-panel .btn-warning{background:#f59e0b;color:#fff}' +
    '#tabuddy-panel .btn-warning:hover{background:#d97706}' +
    ...
    '#tabuddy-panel .toolbar{display:flex;gap:6px;margin-bottom:8px}' +
    '#tabuddy-panel .toolbar button{all:revert;flex:1;padding:5px 0;border-radius:6px;border:1px solid #ddd;background:#fff;cursor:pointer;font-size:12px;transition:all .15s}' +
    '#tabuddy-panel .toolbar button:hover{border-color:#4f46e5;color:#4f46e5}';
  ...
  function clickEditButton() {
    ...
    for (var i = 0; i < all.length; i++) {
      ...
      if (text === '修改') {
        if (all[i].tagName === 'BUTTON' || all[i].tagName === 'A') {
          all[i].click();
        } else {
    ...
    textareas.forEach(function (ta) {
      ...
      for (var i = 0; i < 5; i++) {
        ...
        for (var j = 0; j < labels.length; j++) {
          ...
          if (txt.indexOf(labelText) !== -1 && txt.length < 50) {
            found = true;
            break;
          }
        }
  ...
  function fillTextarea(ta, value) {
    ...
    var enabled = !ta.disabled && ta.readOnly !== true;
    if (!enabled) {
      var editClicked = clickEditButton();
      if (editClicked) {
        showStatus('已自动点击「修改」按钮，请重试填写', 'info');
      } else {
        showStatus('文本框为只读，请手动点击页面上的「修改」按钮后重试', 'error');
      }
      return false;
    }
  ...
  function getEditBtnStatus() {
    var all = document.querySelectorAll('button, a, span, div');
    for (var i = 0; i < all.length; i++) {
      var text = (all[i].textContent || '').trim();
      if (text === '修改') return true;
    }
    return false;
  }
  ...
  panel.innerHTML =
    ...
    '<div class="body">' +
    '<div class="toolbar">' +
    '<button id="tabuddy-btn-edit" class="' + (getEditBtnStatus() ? '' : 'disabled') + '">🔓 点击修改</button>' +
    '<button id="tabuddy-btn-match">🔍 匹配学生</button>' +
    '</div>' +
    '<div class="section">' +
    '<label>📥 第一步：粘贴数据</label>' +
  ...
  var editBtn = document.getElementById('tabuddy-btn-edit');
  var matchBtn = document.getElementById('tabuddy-btn-match');

  editBtn.addEventListener('click', function () {
    var clicked = clickEditButton();
    if (clicked) {
      showStatus('✅ 已点击「修改」按钮，文本框现在应该可编辑了', 'success');
    } else {
      showStatus('⚠️ 未找到「修改」按钮，请手动检查页面', 'error');
    }
  });

  matchBtn.addEventListener('click', function () {
    if (!currentData || !currentData.students) {
      showStatus('请先粘贴数据', 'error');
      return;
    }
    var names = currentData.students.map(function (s) { return s.name; });
    var found = 0;
    var allEls = document.querySelectorAll('button, a, span, div, li');
    allEls.forEach(function (el) {
      var text = (el.textContent || '').trim();
      if (names.indexOf(text) !== -1 && text.length >= 2) {
    ...
    if (found > 0) {
      showStatus('✅ 已高亮标记 ' + found + ' 个匹配的学生元素', 'success');
    } else {
      showStatus('⚠️ 未找到匹配的学生姓名，请确认页面已加载学生列表', 'error');
    }
  });
  ...
  function parseData() {
    ...
    try {
      ...
      showStatus('数据解析失败：请确认已复制完整数据', 'error');
    }
  }
  ...
  function renderStudents() {
    ...
    if (currentIndex < 0 || currentIndex >= currentStudents.length) {
      selectStudent(0);
    } else {
  ...
  function selectStudent(idx) {
    ...
    if (student && student.feedback) {
      ...
      previewBox.textContent = student ? student.name + ' 暂无反馈内容' : '请选择学生';
      previewBox.className = 'preview-box empty';
  ...
  fillBtn.addEventListener('click', function () {
    ...
    var field = targetSelect.value;
    if (!field) { showStatus('请选择填写目标', 'error'); return; }

    var textareas = findTextareasByLabel(field);
    if (textareas.length === 0) {
      clickEditButton();
      setTimeout(function () {
        var retry = findTextareasByLabel(field);
        if (retry.length > 0) {
          var ok = fillTextarea(retry[0], student.feedback);
          if (ok) showStatus('✅ 已填入「' + student.name + '」的反馈到「' + field + '」', 'success');
        } else {
          showStatus('未找到「' + field + '」输入框，请先点击「点击修改」试试', 'error');
        }
      }, 300);
      return;
    }
    var ok = fillTextarea(textareas[0], student.feedback);
    if (ok) showStatus('✅ 已填入「' + student.name + '」的反馈到「' + field + '」', 'success');
  });
  ...
  showStatus('📋 已启动！先点击「点击修改」解锁文本框，然后粘贴数据', 'info');
})();
