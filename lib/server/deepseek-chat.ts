...
export async function chatWithAI(
  ...
  try {
    ...
    console.error('DeepSeek chat API call failed:', (error as Error).message)
    return '抱歉，我现在有点卡顿，你稍等一下再问我好吗？或者你可以直接在工作台里找找看相关功能~'
  }
}

function formatClassDataSnapshot(classData?: Record<string, unknown>): string {
  if (!classData) return ''
...
function buildSystemPrompt(context?: ChatContext): string {
  ...
  const classDataSnapshot = formatClassDataSnapshot(context?.classData)
  const workflowTimeRule = buildWorkflowTimeRule(context)
...
function buildWorkflowTimeRule(context?: ChatContext): string {
...
function formatContextInfo(context?: ChatContext): string {
