...
export function matchKnowledge(input: string): MatchResult {
  const normalized = input.toLowerCase().replace(/\s+/g, '')

  const scored = knowledgeBase
    .map((entry) => {
      const score = calculateScore(normalized, entry)
      return { entry, score }
    })
...
function calculateScore(normalized: string, entry: KnowledgeEntry): number {
  ...
  for (const keyword of entry.keywords) {
    ...
    if (normalized.includes(normalizedKeyword)) {
      ...
      const matchRatio = fuzzyMatchRatio(normalized, normalizedKeyword)
      score = matchRatio * 60
    }
...
function fuzzyMatchRatio(text: string, keyword: string): number {
