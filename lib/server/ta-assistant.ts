import { matchKnowledge, getAllKnowledgeEntries, MatchResult } from './knowledge-matcher'
import { chatWithAI, ChatContext, ChatHistoryItem } from './deepseek-chat'
...
export async function chat(request: ChatRequest): Promise<ChatResponse> {
  const matchResult: MatchResult = matchKnowledge(request.message)
  ...
  const reply = await chatWithAI(request.message, context, request.history)
